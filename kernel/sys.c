					      ...
					      unsigned long arg5)
{
	...
	case PR_SCHED_LLC_AGGR_TOLERANCE_NR:
		if (val > 100)
			return -EINVAL;
		if (val < 0 && val != PR_SCHED_LLC_AGGR_TOLERANCE_DEFAULT)
			return -EINVAL;
		WRITE_ONCE(current->mm->llc_aggr_tolerance_nr, val);
		break;
	case PR_SCHED_LLC_AGGR_TOLERANCE_SIZE:
		...
		if (val < 0 && val != PR_SCHED_LLC_AGGR_TOLERANCE_DEFAULT)
			return -EINVAL;
		WRITE_ONCE(current->mm->llc_aggr_tolerance_size, val);
		break;
	...
	case PR_SCHED_LLC_AGGR_TOLERANCE_OVERLOAD_PCT:
		if (val > 100 &&
		    val != PR_SCHED_LLC_AGGR_TOLERANCE_DEFAULT)
			return -EINVAL;
		if (val == PR_SCHED_LLC_AGGR_TOLERANCE_DEFAULT)
			WRITE_ONCE(current->mm->llc_aggr_tolerance_overload_pct, -1);
		else
			WRITE_ONCE(current->mm->llc_aggr_tolerance_overload_pct, val);
		break;
	default:
		return -EINVAL;
	}
					      ...
					      unsigned long arg5)
{
	...
	switch (option) {
	case PR_SCHED_LLC_AGGR_TOLERANCE_NR:
		ret = READ_ONCE(current->mm->llc_aggr_tolerance_nr);
		break;
	case PR_SCHED_LLC_AGGR_TOLERANCE_SIZE:
		ret = READ_ONCE(current->mm->llc_aggr_tolerance_size);
		break;
	case PR_SCHED_LLC_AGGR_TOLERANCE_FLAGS:
		ret = 0;
		...
		break;
	case PR_SCHED_LLC_AGGR_TOLERANCE_OVERLOAD_PCT:
		ret = READ_ONCE(current->mm->llc_aggr_tolerance_overload_pct);
		break;
	default:
		return -EINVAL;
	}

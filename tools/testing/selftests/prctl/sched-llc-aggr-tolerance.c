...
#define PR_GET_SCHED_LLC_AGGR_TOLERANCE			83
#define PR_SET_SCHED_LLC_AGGR_TOLERANCE			84
...
TEST(flags_and_inheritance)
{
	...
	ret = llc_tol_set(PR_SCHED_LLC_AGGR_TOLERANCE_OVERLOAD_PCT, 12);
	ASSERT_EQ(ret, 0);
...
TEST(execve_inheritance)
{
	...
	ret = llc_tol_set(PR_SCHED_LLC_AGGR_TOLERANCE_OVERLOAD_PCT, 23);
	ASSERT_EQ(ret, 0);
...
int main(int argc, char **argv)
{
	...
	if (mode && *mode) {
		...
		if (nr != expect_nr || size != expect_size || overload != expect_overload)
			return 1;
		return 0;
	}
